...
describe('Teste o componente <Foods.js>',
  () => {
    ...
    it('Verifica se todos os ingredientes de uma receita de comida possuem um checkbox',
      async () => {
        renderWithRouter(<App />, routeToRecipe);
        ...
        const evenIngredientArray = await screen
          .findAllByTestId(/[02468]-ingredient-step/g);
        const unevenIngredientArray = await screen
          .findAllByTestId(/[1357]-ingredient-step/g);
    ...
    it('Verifica se é possível marcar todos os passos da receita de comida', async () => {
      renderWithRouter(<App />, routeToRecipe);
    ...
    it('Verifica se o nome do ingrediente e "riscado" da lista', async () => {
      ...
      const firstCheckbox = await screen.findByText('2Eggs');
